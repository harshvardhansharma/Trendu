...
public class ElasticManager {
	...
	private static void put(Map<List<String>, List<String>> map) throws IOException {
		for (Entry<List<String>, List<String>> item : map.entrySet()) {
			XContentBuilder builder = jsonBuilder().startObject().field("keywords", item.getKey())
					.field("categories", item.getValue()).endObject();
	...
	private static SearchResponse search(String field, String value) {
		...
		SearchResponse response = client.prepareSearch().setIndices("categories").setTypes("category")
				.setMinScore(0.55f).setSearchType(SearchType.QUERY_AND_FETCH)
				.setQuery(QueryBuilders.fuzzyQuery(field, value)).execute().actionGet();
		return response;
	}
